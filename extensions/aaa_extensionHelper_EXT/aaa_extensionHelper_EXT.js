class Extension {
  ...
  mod(DBM) {
    const extensionHelper = {
      ...
      async requireModule(packageName, projectName) {
        ...
        try {
          ...
          const nodeModulesPath = join(__dirname, 'aaa_extensionHelper_EXT', projectName, 'node_modules', packageName);
          extensionHelper.log.success(
            `(aaa_extensionHelper_EXT Auto Module Installer) ~ Successfully Installed ${packageName}. Note you may need to restart your bot.`,
          );
          return require(nodeModulesPath);
        }
      },
      async autoUpdater(data) {
        ...
        extensionHelper.log.warn(
          `${depInfo.name} is out of date. (aaa_extensionHelper_EXT Auto Updater) ~ Updating ${depInfo.name}`,
        );
    ...
    extensionHelper.autoUpdater({
      depInfo:
        'https://gist.githubusercontent.com/greatplainsmodding/cbcaf9da3a4ddb08bc06fad3f1f32aaf/raw/aaa_greatPlainsModdingDeps_EXT.json',
      depFile:
        'https://gist.githubusercontent.com/greatplainsmodding/cbcaf9da3a4ddb08bc06fad3f1f32aaf/raw/aaa_greatPlainsModdingDeps_EXT.js',
      version: '1.0.4',
    });
