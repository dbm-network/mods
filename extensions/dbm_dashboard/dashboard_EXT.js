  ...
  mod: async (DBM) => {
    ...
    Dashboard.updateExtension = async () => {
      ...
      if (!fs.existsSync(extHelperPath)) {
        ...
        const url =
          await 'https://raw.githubusercontent.com/dbm-network/extensions/master/extensions/aaa_extensionHelper_EXT/aaa_extensionHelper_EXT.js';
        ...
        console.log(
          chalk.green('Successfully installed aaa_extensionHelper_EXT.js, note you may need to restart your bot.'),
        );
        setTimeout(() => {}, 1000);
      }
    ...
    Dashboard.checkActions = async function checkActions() {
      try {
    ...
    Dashboard.storeData = function storeData(fileName, dataName, data) {
      if (!fileName) {
        return console.log('storeData("fileName", "dataName", "data")');
      }
      if (!dataName) {
        return console.log('storeData("fileName", "dataName", "data")');
      }
      ...
      try {
        ...
        const resourceFilePath = path.join(__dirname, 'dashboard_EXT', `${fileName}.json`);
    ...
    Dashboard.retrieveData = function retrieveData(fileName, dataName) {
      try {
        if (!fs.existsSync(path.join(__dirname, 'dashboard_EXT'))) {
          fs.mkdirSync(path.join(__dirname, 'dashboard_EXT'));
        }
    ...
    Dashboard.retrieveFile = function retrieveFile(fileName) {
      try {
        if (!fs.existsSync(path.join(__dirname, 'dashboard_EXT'))) {
          fs.mkdirSync(path.join(__dirname, 'dashboard_EXT'));
        }
    ...
    Dashboard.loadMods = function loadMods() {
      fs.readdirSync(Dashboard.Actions.modsLocation).forEach((dir) => {
        ...
        if (modData.cssFiles) {
          modData.cssFiles.forEach((cssFile) => {
            Dashboard.app.get(`/${modData.name}/css/${cssFile}`, (req, res) => {
    ...
    Dashboard.loadRoutes = function loadRoutes() {
          ...
          routeData.scriptFiles.forEach((file) => {
            ...
            if (routeData.cssFiles) {
              routeData.cssFiles.forEach((cssFile) => {
                Dashboard.app.get(`/${routeData.name}/css/${cssFile}`, (req, res) => {
                  res.sendFile(path.join(__dirname, 'dashboard_EXT', 'actions', 'routes', dir, cssFile));
                });
              });
            }
            if (routeData.loginRequired) {
              Dashboard.app.get(routeData.routeURL, Dashboard.checkAuth, (req, res) => {
              ...
              Dashboard.app.get(routeData.routeURL, (req, res) => {
                const renderData = fileData.run(DBM, req, res, Dashboard);
                res.render(webFile, {
                  data: renderData,
                });
    ...
    Dashboard.loadExtensions = function loadExtensions() {
      fs.readdirSync(Dashboard.Actions.extensionLocation).forEach((dir) => {
        ...
        if (extensionData.cssFiles) {
          extensionData.cssFiles.forEach((cssFile) => {
            Dashboard.app.get(`/${extensionData.name}/css/${cssFile}`, (req, res) => {
              res.sendFile(path.join(__dirname, 'dashboard_EXT', 'actions', 'mods', dir, cssFile));
            });
          });
        }
    ...
    Dashboard.checkAuth = function checkAuth(req, res, next) {
    ...
    Dashboard.randomString = function randomString() {
      let string = '';
    ...
    Dashboard.verifyConfig = function verifyConfig() {
      const errors = [];
      let { settings } = Dashboard;

      if (settings.runSetup) {
        Dashboard.app.get('/', (_req, res) => {
          res.render('setup', {
      ...
      if (!settings.port) {
        errors.push('Invalid port, please check your config.');
      }
      ...
      if (!settings.clientSecret) {
        errors.push('Invalid client secret, please check your config.');
      }
      if (!settings.callbackURL) {
        errors.push('Invalid callback url, please check your config.');
      }
      return errors;
    };

    Dashboard.appSettings = function appSettings() {
    ...
    Dashboard.passport = function _passport() {
    ...
    Dashboard.loginRoute = function loginRoute() {
      Dashboard.app.get(
        '/login',
    ...
    Dashboard.onReady = function onReady() {
