    ...
    */
    ...
    mod: async function (DBM) {
        ...
        if (!require("fs").existsSync(filePath)) {
            ...
            let url = await "https://raw.githubusercontent.com/dbm-network/extensions/master/extensions/aaa_extensionHelper_EXT/aaa_extensionHelper_EXT.js";
        ...
        Dashboard.checkActions = async function () {
            try {
                ...
                let modsFetched = await fetch("https://api.github.com/repos/greatplainsmodding/DBM-Dashboard-Mods/git/trees/master").then(res => res.json().then(data => data.tree));
                ...
                for (let mod of modsFetched) {
                    ...
                    if (!require("fs").existsSync(filePath)) {
                        ...
                        let fetchedMod = await fetch(mod.url).then(res => res.json().then(data => data.tree));
                        ...
                        for (let file of fetchedMod) {
                            ...
                            if (!require("fs").existsSync(filePath)) {
                                let modFile = await fetch("https://raw.githubusercontent.com/greatplainsmodding/DBM-Dashboard-Mods/master/" + mod.path + "/" + file.path).then(res => res.text());
        ...
        const express = await DBM.extensionHelper.requireModule("express", "dashboard_EXT"),
            bodyParser = await DBM.extensionHelper.requireModule("body-parser", "dashboard_EXT"),
            cookieParser = await DBM.extensionHelper.requireModule('cookie-parser', "dashboard_EXT"),
            ejs = await DBM.extensionHelper.requireModule('ejs', "dashboard_EXT"),
            Strategy = await DBM.extensionHelper.requireModule('passport-discord', "dashboard_EXT"),
            session = await DBM.extensionHelper.requireModule('express-session', "dashboard_EXT"),
            path = await DBM.extensionHelper.requireModule('path', "dashboard_EXT"),
            passport = await DBM.extensionHelper.requireModule('passport', "dashboard_EXT"),
            chalk = await DBM.extensionHelper.requireModule("chalk", "dashboard_EXT"),
            figlet = await DBM.extensionHelper.requireModule("figlet", "dashboard_EXT"),
            url = await require('url');
                    ...
                    routeData.scriptFiles.forEach(file => {
                        ...
                        if (routeData.cssFiles) {
                            routeData.cssFiles.forEach(cssFile => {
                                Dashboard.app.get(`/${routeData.name}/css/${cssFile}`, function (req, res, next) {
                                    res.sendFile(require("path").join(__dirname, "dashboard_EXT", "actions", "routes", dir, cssFile))
                                });
                            });
                        };
                        if (routeData.loginRequired) {
                            Dashboard.app.get(routeData.routeURL, Dashboard.checkAuth, function (req, res, next) {
