module.exports = {
    ...
    name: "Send Message for channel",
    //----------------------------------------------------------------------------------

    //----------------------------------------------------------------------------------
    // Here you can configure what section you want your mod to show up on the dashboard / admin panel. 
    //If this is an extension or route modifier, you can leave it blank.
    section: "Dashboard",
    //----------------------------------------------------------------------------------
    ...
    short_description: "Send a personalized message to a specific channel",
    //----------------------------------------------------------------------------------
    ...
    run: async (app, config, DBM, client, req, res, server) => {
     let channel;
        ...
        try {
            ...
            if (!channel) return req.user.log = 'I couldn\'t find this server, please make sure you have the right ID or name.';
    
    
            channel.send(req.body.message);
            req.user.log = `Successfully sent the message to ${channel.name} | ${server.name}`;   
        } catch (error) {
