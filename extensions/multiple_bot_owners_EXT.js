...
<div class="container has-text-centered"><br>
  ...
  mod(DBM) {
    ...
    try {
      ...
      Actions.checkConditions = function checkConditions(guild, member, user, cmd) {
        if (cmd == null) return true;
        const isServer = Boolean(guild && member);
        const restriction = parseInt(cmd.restriction, 10);
        const { permissions } = cmd;
        const msgLike = { guild, member, author: user };
        switch (restriction) {
          case 0:
            if (isServer) {
              return this.checkPermissions(msgLike, permissions);
            }
            return true;

          case 1:
            return isServer && this.checkPermissions(msgLike, permissions);
          case 2:
            return isServer && guild && guild.ownerId && member && guild.ownerId === member.id;
          case 3:
            return !isServer;
          case 4:
            return (
