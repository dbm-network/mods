    ...
    cronScheduler.setupCrons = function () {
      ...
      for (const event of Bot.$evts['Cron Scheduler']) {
        try {
          ...
          if (!cron.validate(cronString)) return console.error(`[Cron Scheduler] Invalid cron string for '${eventName}': '${cronString}'`)
          if (!cronScheduler.isValidTimeZone(timeZone)) return console.error(`[Cron Scheduler] Invalid Timezone for '${eventName}': '${timeZone}'`)
