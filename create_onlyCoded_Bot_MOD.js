...
bot.on("message", async message => {
    ...
    if (command === \`\${prefix}await\`) {
        ...
        message.channel.send(\`Await completed! Answer: EliteArtz, Familiar Answers collected: "\${messages1.map(message => message.content).join(", ")}"\`);
...
bot.on("message", async message => {
    ...
    if (command === \`\${prefix}voter\`) {
        ...
        try {
            let msg = await message.channel.send("Vote! *You have 15 seconds to vote!*");
            await msg.react(x);
