...
<div style="width: 550px; height: 350px; overflow-y: scroll;">
  ...
  <div style="padding-top: 8px;">
	...
	action (cache) {
		...
		const { message } = data;
		if (!target || target.type !== 'news' || !message) return;
		if (djsversionoutput && djsversionoutput[1] && djsversionoutput[1] < "12.4.0") throw new Error(`You need at least Discord.js version 12.4.0 to use this mod. Your Discord.js version is: ${djsversionoutput[1]}`);
		
		if (Array.isArray(target)) {
			this.callListFunc(target, 'send', [this.evalMessage(message, cache)]).then((msg) => {
				msg.crosspost().then((m) => {
					const varName2 = this.evalMessage(data.varName2, cache);
					const storage = parseInt(data.storage);
					this.storeValue(m, storage, varName2, cache);
