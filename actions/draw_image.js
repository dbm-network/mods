...
action: function(cache) {
    ...
    Jimp.read(image2, function (err, image3) {
	if(mask === "2") {
		image1.mask(image3, x, y);
	} else if(mask === "1") {
		_this.getDBM().Images.drawImageOnImage(image1, image2, x, y);
	} else {
		image1.composite(image3, x, y);
	}
				image1.getBuffer(Jimp.MIME_PNG, (error, image4) => {
			_this.storeValue(image4, storage, varName, cache)
