module.exports = {
  ...
  subtitle(data, presets) {
    ...
    return `Draw new winners (${winnerCount}), for giveaway (${giveaway})`;
  },
  ...
  fields: ['storage', 'varName', 'giveawayIdInput', 'winnerCount', 'storage2', 'varName2'],
...
<div style="padding-top: 10px;">
  ...
  <input id="winnerCount" class="round" type="text" placeholder="Leave blank for default...">
</div>
...
<store-in-variable dropdownLabel="Store In" selectId="storage2" variableContainerId="varNameContainer2" variableInputId="varName2"></store-in-variable>
  ...
  async action(cache) {
    ...
    const gWinnerCount = this.evalMessage(data.winnerCount, cache);
    // /////////////////////////////////////////////////////

    const newWinner = GiveawayReroll({
      storage: './data/giveaways.json',
      giveawayId: gId,
      winnerCount: gWinnerCount,
    });

    const varName2 = this.evalMessage(data.varName2, cache);
    const storage = parseInt(data.storage2, 10);
    this.storeValue(newWinner, storage, varName2, cache);
