module.exports = {
	...
	subtitle: function(data) {
		...
		//---------------------------------------------------------------------
		 ...
		 author: "Lasse, MrGold, NetLuis & RigidStudios", // UI fixed by MrGold
	...
	html: function(isEvent, data) {
		...
		<div>
			<p>
				<u>Mod Info:</u><br>
				Created by Lasse, MrGold, NetLuis & RigidStudios!
			</p>
		</div><br>
	<div>
	...
	<div>
		<div class="" id="changeText" style="float: left; width: 80%;">
			Change to:<br>
	...
	action: function(cache) {
		...
		const actionNum = parseInt(cache.index) + 1;
		...
		switch(toChange) {			// Made into a switch because RigidStudios found the 'else if' disgusting.
			...
			case 0:
				channel.edit({name: newState}, reason);
				break;

			case 1:
				if (channelType == 0) {
				channel.edit({topic: newState}, reason);
			}	else {
				console.log(`Error with Event "Edit Channel", Action #${actionNum}\nCan't edit Topic of a Voice Channel.`)	// All these things added by RigidStudios
			} break;

			case 2:
				channel.edit({position: newState}, reason);
				break;

			case 3:
				if (channelType == 1) {
				channel.edit({bitrate: parseInt(newState)}, reason);
			} else {
				console.log(`Error with Event "Edit Channel", Action #${actionNum}\nCan't edit Bitrate of a Text Channel.`)
			}	break;

			case 4:
				if (channelType == 1) {
				channel.edit({userLimit: parseInt(newState)}, reason);
			} else {
				console.log(`Error with Event "Edit Channel", Action #${actionNum}\nCan't edit User Limit of a Text Channel.`)
			}	break;

			case 5:
				channel.setParent(newState);	// Lasse: 1.8.7
				break;

			case 6:
				if (channelType == 0) {
				if (newState >= 0 && newState <= 120) {	// Lasse: 1.9 | MrGold & NetLuis: 1.9.4
					...
					console.log(`Error with Event "Edit Channel", Action #${actionNum}\nRate Limit value must be between 0 and 120.`)
				}
			} else {
			...
			case 7:	// RigidStudios
				...
				console.log(`Error with Event "Edit Channel", Action #${actionNum}\nCan't edit NSFW of a Voice Channel.`)
			} break;
