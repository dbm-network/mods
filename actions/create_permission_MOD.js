module.exports = {
	...
	subtitle: function(data) {
		const target = ["Role Permissions", "Category Channel Permissions", "Text Channel Permissions", "Voice Channel Permissions"];
		const variables = ["", "Temp Variable", "Server Variable", "Global Variable"];
		return `For ${target[parseInt(data.targetType)]} ${variables[parseInt(data.storage)]} (${data.varName})`;
	},
	...
	action: function(cache) {
		...
		const Permissions = this.getDBM().DiscordJS.Permissions;
		let permissions = {};
		switch(type) {
			case 0:
