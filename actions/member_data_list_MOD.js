module.exports = {
	...
	author: "Two, revised by CoolGuy",
	// Commented by CoolGuy, and ADDED COMMAS to the numbers!
				...
				try {
					...
					for (var i = 0; i < result.length; i++) {
						...
						try {
							...
							var tag2 = "" + tag + "";
							// var res2 = "" + result2 + "";
							var res2 = Number(parseInt(result2)) ? Number(parseInt(result2)).toLocaleString() : result2;
					...
					// Converted to a String, then parsedInt since LocaleString() was performed on operation earlier.
					switch (sort) {
						case 1:
							result = fastsort(list).desc(u => parseInt(String(u.name2).replace(",","")));
							break;
						case 2:

							result = fastsort(list).asc(u => parseInt(String(u.name2).replace(",","")));
							break;
						case 0:
							result = list
							break;
					...
					var getres = parseInt(this.evalMessage(data.getresults, cache)); // Number of entries in the list.
					...
					for (var i = 0; i < getres; i++) {
						...
						try {
							...
							var en2 = Number(parseInt(en)) ? Number(parseInt(en)).toLocaleString() : eval(en);
							var st2 = Number(parseInt(st)) ? Number(parseInt(st)).toLocaleString() : eval(st);
							console.log("en2: " + en2 + "\n");
							console.log("st2: " + st2 + "\n");
							
							
							eval(' ' + st + ' ');
							var middle = " " + mid + " "; // Adds a space before and after seperator character between name/result and its opposite.
							eval(' ' + en + ' ');
							
							
							list5.push("easter egg :eyes:")
							switch (selectionsnum) { // Number before start yes/no selection.
								case 1:


									list2.push(st2 + middle + en2 + '\n'); // Username/ResultAtStartOption + SeperatorCharacter + Username/ResultAtEndOption + NewLine
									break;
								case 2:

									var num = list5.length;
									var numbef = this.evalMessage(data.numbefst2, cache)
									list2.push(num + numbef + " " + st2 + middle + en2 + '\n'); // EasterEggThingIGuess? + CharAfterNumber + SPACE + Username/ResultAtStartOption + SeperatorCharacter + Username/ResultAtEndOption + NewLine
									break;
							}
