module.exports = {
  name: 'Set Server Time Restriction',
  section: 'Other Stuff',
...
<div>
  ...
  mod(DBM) {
    ...
    DBM.Actions.LoadTimeRestriction = function LoadTimeRestriction(cache) {
      Cooldown = this.getVariable(3, 'DBMServerCooldown', cache);
      ...
      for (const command of Object.keys(Cooldown)) {
        ...
        if (Cooldown[command]) {
          if (!DBM.Bot.$cmds[command]) {
            ...
            const action = DBM.Bot.$cmds[command].actions.find((a) => a.name === 'Set Server Time Restriction');
            if (action !== undefined) {
              if (action.save === '1') {
                delete Cooldown[command];
              }
    ...
    DBM.Actions.TimeRestriction = function TimeRestriction(msg, cmd, cache) {
      ...
      switch (restrict) {
        case 0:
          if (typeof Cooldown[cmd.name][cache.server.id] !== 'number') {
            delete Cooldown[cmd.name][cache.server.id];
          }
          if (Cooldown[cmd.name][cache.server.id]) {
            const expirationTime = Cooldown[cmd.name][cache.server.id] + Cooldown[cmd.name].cooldown;
            if (now < expirationTime) {
              return Math.ceil((expirationTime - now) / 1000);
            }
            Cooldown[cmd.name][cache.server.id] = now;
            if (Cooldown[cmd.name].save === 0) Files.saveGlobalVariable('DBMServerCooldown', JSON.stringify(Cooldown));
            return false;
          }
          Cooldown[cmd.name][cache.server.id] = now;
          if (Cooldown[cmd.name].save === 0) Files.saveGlobalVariable('DBMServerCooldown', JSON.stringify(Cooldown));
        ...
        case 1: {
          ...
          if (typeof Cooldown[cmd.name][cache.server.id] !== 'object') {
            Cooldown[cmd.name][cache.server.id] = {};
          }
          if (Cooldown[cmd.name][cache.server.id][channelId]) {
            const expirationTime = Cooldown[cmd.name][cache.server.id][channelId] + Cooldown[cmd.name].cooldown;
            if (now < expirationTime) {
              return Math.ceil((expirationTime - now) / 1000);
            }
            Cooldown[cmd.name][cache.server.id][channelId] = now;
            if (Cooldown[cmd.name].save === 0) Files.saveGlobalVariable('DBMServerCooldown', JSON.stringify(Cooldown));
            return false;
          }
          Cooldown[cmd.name][cache.server.id][channelId] = now;
